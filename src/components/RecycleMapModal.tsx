...
export default function RecycleMapModal({
  ...
  return (
    <motion.div
      ...
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
      className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6 md:p-8 bg-[#362A1F]/50 backdrop-blur-xl"
      onClick={onClose}
    >
      <motion.div
        ...
        ref={surfaceRef}
        className="w-full max-w-[min(1520px,96vw)] h-[90vh] bg-[#FDFBF7] border border-[#DECFBE] rounded-3xl shadow-[0_20px_50px_rgba(54,42,31,0.25)] flex flex-col relative overflow-hidden"
      >
        <motion.button
          onClick={onClose}
          whileHover={{ scale: 1.04, y: -1 }}
          whileTap={{ scale: 0.96 }}
          className="absolute top-5 right-5 z-50 p-2.5 bg-[#F4F0E8] hover:bg-[#EAE3D4] rounded-full text-[#4A3D30] shadow-sm border border-[#DECFBE] transition-all hover:scale-105"
        >
          <X className="w-5 h-5" />
        </motion.button>

        <div className="px-5 py-4 md:px-6 border-b border-[#DECFBE] bg-[#FDFBF7] flex flex-col gap-3 z-40 relative">
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-3 pr-12">
            <div className="flex items-center gap-3" data-map-hero>
              <div className="w-9 h-9 rounded-full bg-[#F4F0E8] flex items-center justify-center text-[#986E4B] border border-[#DECFBE]">
                <MapIcon className="w-4 h-4" />
              </div>
              <div className="min-w-0">
                <h3 className="text-lg md:text-xl font-serif text-[#362A1F] tracking-wide">附近回收网络</h3>
                <p className="text-sm text-[#7F6B58] truncate max-w-[min(62vw,760px)]">正在查看: {activeLocation.label}</p>
              </div>
            </div>

            <div
              className="flex flex-wrap gap-x-4 gap-y-1 text-xs font-bold text-[#6C5B49] bg-[#F4F0E8] px-3 py-2 rounded-full border border-[#DECFBE] self-start lg:self-auto"
              data-map-hero
            >
        ...
        <div className="flex-1 relative bg-[#EAE3D4] overflow-hidden" data-map-panel>
          ...
          <div className="absolute inset-x-4 top-4 z-[500] flex flex-col gap-3 pointer-events-none">
            <div
              className="pointer-events-auto w-full max-w-[840px] bg-[rgba(253,251,247,0.92)] border border-[#DECFBE] rounded-2xl overflow-hidden shadow-[0_18px_40px_rgba(54,42,31,0.12)] backdrop-blur-xl"
              data-map-panel
            >
              <div className="flex flex-col md:flex-row">
                <input
                  ...
                  className="min-w-0 flex-1 px-5 py-4 bg-transparent outline-none text-[#362A1F] placeholder:text-[#BAAFA0]"
                />
                <div className="grid grid-cols-2 border-t md:border-t-0 md:border-l border-[#DECFBE] bg-[#F7F2EA]">
                  <motion.button
                    ...
                    whileHover={{ y: -1 }}
                    whileTap={{ scale: 0.98 }}
                    className="px-4 py-4 text-[#986E4B] hover:text-[#6C4B30] hover:bg-[#F4F0E8] flex items-center justify-center gap-2 text-sm font-medium transition-colors disabled:opacity-60 disabled:cursor-wait"
                  >
                    {isSearching ? (
                  ...
                  <motion.button
                    ...
                    whileHover={{ y: -1 }}
                    whileTap={{ scale: 0.98 }}
                    className="px-4 py-4 text-[#986E4B] hover:text-[#6C4B30] hover:bg-[#F4F0E8] flex items-center justify-center gap-2 text-sm font-medium transition-colors border-l border-[#DECFBE] disabled:opacity-60 disabled:cursor-wait"
                  >
                    {isLocating ? (
                      <LoaderCircle className="w-4 h-4 animate-spin" />
                    ) : (
            ...
            <div
              className="pointer-events-auto ml-auto w-full md:w-[300px] max-w-full bg-[rgba(253,251,247,0.9)] border border-[#DECFBE] rounded-2xl p-4 shadow-[0_18px_40px_rgba(54,42,31,0.12)] backdrop-blur-xl max-h-[min(42vh,360px)] overflow-auto"
              data-map-panel
            >
